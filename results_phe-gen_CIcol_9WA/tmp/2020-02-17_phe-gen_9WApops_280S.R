...
# now do the same thing within species, using location as a random effect
...
for (spi in c("col","gam")){
  ...
  # report
  print(paste(spi,nrow(data), mod_pval_chisq_v_null, mod_variables,
              signif(mean(data$CNV),3), signif(sd(data$CNV)/sqrt(nrow(data)),3) ) )
  print(table(data$phenotype))
...
# significance of the BKE model relative to full
mod_signif = anova(mod_BKE$final.model, mod_full, test = 'Chisq')
mod_variables = paste(rownames(mod_BKE$final.sig), collapse = ",")
mod_pval_chisq_v_null = mod_signif$`Pr(>Chisq)`[2]
