...
kmerbat_fn = "full_sig_merged_kmers.blat"
...
kmerbat = pd.read_csv(kmerbat_fn, sep='\t', header=None)
kmerbat.columns = ["kmer","chrom","id","alilen","mismatch","gaps","qstart","qend","sstart","ssend","eval","bitscore"]
...
for i,chrom in enumerate(chromlist):
	...
	# where does the independent kmer map?
	kmerbai_ix = np.where(kmerbai["kmer"].values == "Sig_kmer1993")[0]
	...
	for p,start in enumerate(chrstarts):
		count = len(
			kmerbai[ np.logical_and( 
				kmerbai["sstart"] >= chrstarts[p] , 
				kmerbai["sstart"] <= chrstops[p] ) ] )
		mapfreqs[p] = count
	...
	plt.plot(kmerbai["sstart"].values[kmerbai_ix]/1e6, np.repeat(450, len(kmerbai_ix)), color="orange", marker="v", linewidth=0, markersize=5)
	plt.title("%s" % chrom)
	plt.xlabel("Mb")
